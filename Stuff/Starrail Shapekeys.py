bl_info = {
    "name": "Starrail Shapekeys",
    "author": "Me",
    "version": (1, 0, 0),
    "blender": (3, 6, 9),
    "location": "3D View > Sidebar > Starrail",
    "description": "Manages shape keys for models.",
    "warning": "",
    "doc_url": "",
    "support": 'COMMUNITY',
    "category": "VRC"
...
def manage_shape_keys(context):
    ...
    create_shape_key_from_mix("Blink", {"00_Close01_Eye": 1})
    create_shape_key_from_mix("Blink Straight", {"00_Close02_Eye": 1})
    create_shape_key_from_mix("Blink Happy", {"00_Close03_Eye": 1})

    # Create Ah, Oh, and Ch
    create_shape_key_from_mix("Ah", {"Mouth_00_Phoneme_cn_ao": 1})
    create_shape_key_from_mix("Oh", {"Mouth_00_Phoneme_cn_u": 1})
    create_shape_key_from_mix("Ch", {"Mouth_00_N": 1})
...
def set_shape_key_order(key_name, index):
    ...
    if obj and obj.data.shape_keys:
        ...
        if key_name in shape_keys:
            # Get the index by using the list index instead of direct access
...
class ShapeKeyManagerOperator(bpy.types.Operator):
    """Creates and sets up shape keys for models."""
    bl_idname = "object.shape_key_manager_operator"
    bl_label = "Setup StarRail Shapekeys"  # Updated button label

    def execute(self, context):
        manage_shape_keys(context)
        return {'FINISHED'}

# Create a new panel in the UI
class ShapeKeyPanel(bpy.types.Panel):
    """Creates a panel in the 3D Viewport sidebar for shape keys"""
    bl_label = "Starrail Shapekey Manager"
    bl_idname = "VIEW3D_PT_shape_key_panel"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_category = 'Starrail'

    def draw(self, context):
        layout = self.layout
        row = layout.row()
        row.operator("object.shape_key_manager_operator", text="Setup StarRail Shapekeys")  # Updated button label
